...
public class AppoinmentRepository : BaseRepository<Appointment>, IAppoinmentRepository
{
    ...
    {
        var x = await FindAllAsync(s => s.PatientId == PatientId && s.Status != "Completed" && !s.IsDeleted, new string[] { "Patient", "Doctor" });
        return x.ToList();
    }
    ...
    {
        var x = await FindAllAsync(s => s.CreatedBy == Convert.ToString(CreatedBy) && !s.IsDeleted, new string[] { "Patient", "Doctor" });
        return x.ToList();
    }
